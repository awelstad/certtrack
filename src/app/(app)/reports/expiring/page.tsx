...
export default async function ExpiringCertsReport({
  searchParams,
}: {
  searchParams: Promise<{ days?: string; type?: string }>
}) {
  const { days: daysParam, type: typeFilter } = await searchParams
  const withinDays = Number(daysParam ?? 30)
  ...
  let query = supabase
    ...
    .select('id, expiry_date, workers(id, first_name, last_name, trade, employer), certification_types(id, name)')
  ...
  const csvCols = [
    ...
    { header: 'Days Remaining',key: 'Days Left' },
    ...
    <>
      ...
      <div className="px-4 py-6 sm:px-6 lg:px-8 max-w-6xl">
        <div className="report-print-hide mb-4 flex items-center justify-between">
          ...
          <ReportExportButtons filename="expiring-certifications" csvRows={rows} csvColumns={csvCols} />
        </div>
        ...
        <ReportPrintHeader
          ...
          subtitle={`Within ${withinDays} days · ${rows.length} record${rows.length !== 1 ? 's' : ''}`}
        />
        ...
        <form method="GET" className="report-print-hide mb-5 flex flex-wrap gap-3">
          <div className="flex rounded-lg border border-slate-200 bg-white overflow-hidden">
            {DAY_OPTIONS.map((d) => (
              <Link
                key={d}
                href={`?days=${d}${typeFilter ? `&type=${typeFilter}` : ''}`}
                className={[
              ...
              <tr key={i} className="hover:bg-slate-50">
                ...
                <Td>
                  <span className={[
                    ...
                    urgent ? 'bg-red-50 border-red-200 text-red-700' : 'bg-yellow-50 border-yellow-200 text-yellow-700',
                  ].join(' ')}>
                    {r['Expiry Date']}
                  </span>
                </Td>
                <Td className={`font-semibold ${urgent ? 'text-red-700' : 'text-yellow-700'}`}>
                  {r['Days Left']}d
                </Td>
                <Td>{r['Job Site']}</Td>
              </tr>
